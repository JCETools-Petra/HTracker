...
class FinancialReportService
{
    ...
    public function getPnL(int $propertyId, int $year, int $month): array
    {
        ...
        // Fetch Current Month Entries (Actual & Budget)
    ...
    private function processCategoryRecursive(FinancialCategory $category, array $context, int $level = 0): array
    {
        ...
        if ($category->hasChildren()) {
            foreach ($category->children as $child) {
                ...
                $data['forecast_current'] += $childData['forecast_current']; 
                
                $data['actual_ytd'] += $childData['actual_ytd'];
                $data['budget_ytd'] += $childData['budget_ytd'];
                $data['forecast_ytd'] += $childData['forecast_ytd'];         
            }
        } else {
            // --- FIX PENTING DIMULAI DI SINI ---
            
            // 1. Ambil data Budget & Forecast DULU dari database (FinancialEntry)
            //    Kita ambil ini APAPUN JENIS KATEGORINYA (Auto/Manual)
            ...
            $data['forecast_current'] = $entryCurrent ? $entryCurrent->forecast_value : 0;
            $data['forecast_ytd'] = $entryYtd ? ($entryYtd->total_forecast ?? 0) : 0;

            // 2. Tentukan data ACTUAL (Apakah dari Auto System atau Input Manual?)
            if ($category->code && isset($context['auto_values']['current'][$category->code])) {
                // Kategori Otomatis (Room Revenue, Breakfast, MICE, dll)
                // Ambil ACTUAL dari Auto Values
                $data['actual_current'] = $context['auto_values']['current'][$category->code];
                $data['actual_ytd'] = $context['auto_values']['ytd'][$category->code];
                
                // DULU: Di sini budget dipaksa jadi 0, SEKARANG: Kita biarkan budget dari step 1 di atas.
            } else {
                // Kategori Manual (Expense, dll)
                // Ambil ACTUAL dari FinancialEntry
    ...
    public function getChartData(int $propertyId, int $year, int $month): array
    {
        ...
        // 1. Data Budget (Ambil SEMUA budget tanpa terkecuali)
        $budgetData = FinancialEntry::query()
            ...
            ->selectRaw('year, month, financial_categories.type, SUM(budget_value) as total_budget')
            ->groupBy('year', 'month', 'financial_categories.type')
            ->get();

        // 2. Data Actual Manual (Exclude kategori otomatis agar tidak double)
        // Kategori otomatis adalah yang punya CODE (ROOM_REV, BREAKFAST_REV, dll)
        $manualActualData = FinancialEntry::query()
            ->join('financial_categories', 'financial_entries.financial_category_id', '=', 'financial_categories.id')
            ->where('financial_entries.property_id', $propertyId)
            ->whereBetween(DB::raw("CAST(CONCAT(year, '-', month, '-01') AS DATE)"), [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
            ->whereNull('financial_categories.code') // Hanya ambil yang tidak punya kode otomatis
            ->selectRaw('year, month, financial_categories.type, SUM(actual_value) as total_actual')
            ->groupBy('year', 'month', 'financial_categories.type')
            ->get();

        // 3. Data Actual Otomatis (Daily Income)
        $dailyIncomeData = DailyIncome::where('property_id', $propertyId)
            ->whereBetween('date', [$startDate, $endDate])
            ->selectRaw('
                YEAR(date) as year, 
                MONTH(date) as month, 
                SUM(total_rooms_revenue + total_fb_revenue + IFNULL(package_income,0) + IFNULL(rental_area_income,0) + IFNULL(others_income,0)) as total_auto_revenue
            ')
            ->groupByRaw('YEAR(date), MONTH(date)')
            ->get();

        // 4. Data Actual Otomatis (MICE)
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            // Budget: Langsung dari tabel Financial Entry (sudah mencakup semuanya karena di query budget tidak difilter)
            $revBudget = $budgetData->where('year', $y)->where('month', $m)->where('type', 'revenue')->first();
            $expBudget = $budgetData->where('year', $y)->where('month', $m)->where('type', 'expense')->first();
            
            $revenueBudget = $revBudget ? $revBudget->total_budget : 0;
            $expenseBudget = $expBudget ? $expBudget->total_budget : 0;

            // Actual: Gabungan Manual + Auto
            $manualRev = $manualActualData->where('year', $y)->where('month', $m)->where('type', 'revenue')->first();
            $manualExp = $manualActualData->where('year', $y)->where('month', $m)->where('type', 'expense')->first();

            $autoDaily = $dailyIncomeData->where('year', $y)->where('month', $m)->first();
            $autoMice = $bookingData->where('year', $y)->where('month', $m)->first();

            // Hitung Total Revenue (Actual)
