...
class FinancialCategorySeeder extends Seeder
{
    ...
    {
        // Hapus kategori lama untuk properti ini agar tidak duplikat
        FinancialCategory::where('property_id', $propertyId)->delete();

        // Ambil data properti untuk pengecekan nama
        ...
        // 1. Room Revenue
        $this->createCategory($propertyId, null, 'Room Revenue', 'ROOM_REV', 'revenue', false, $sortOrder++);

        // 2. Beverage Revenue
        $this->createCategory($propertyId, null, 'Beverage Revenue', 'BEV_REV', 'revenue', false, $sortOrder++);

        // 3. Breakfast Revenue (FIXED LOGIC)
        // Cek "Sunnyday" saja agar kena baik "Sunnydayinn" maupun "Sunnyday Inn"
        if ($property && stripos($property->name, 'Sunnyday') !== false) {
            $this->createCategory($propertyId, null, 'Breakfast Revenue/Incl. Bell & Ermasu', 'BREAKFAST_REV', 'revenue', false, $sortOrder++);
            
            // Debugging: Catat ke log bahwa ini terdeteksi sebagai Sunnyday
            \Log::info("Seeding Special Breakfast for: " . $property->name);
        } else {
            $this->createCategory($propertyId, null, 'Breakfast Revenue', 'BREAKFAST_REV', 'revenue', false, $sortOrder++);
        }

        // 4. Lunch Revenue
        $this->createCategory($propertyId, null, 'Lunch Revenue', 'LUNCH_REV', 'revenue', false, $sortOrder++);

        // 5. Dinner Revenue
        $this->createCategory($propertyId, null, 'Dinner Revenue', 'DINNER_REV', 'revenue', false, $sortOrder++);

        // 6. Package Revenue
        $this->createCategory($propertyId, null, 'Package Revenue', 'PACKAGE_REV', 'revenue', false, $sortOrder++);

        // 7. MICE Revenue
        $this->createCategory($propertyId, null, 'MICE Revenue', 'MICE_REV', 'revenue', false, $sortOrder++);

        // 8. Rental Area
        $this->createCategory($propertyId, null, 'Rental Area', 'RENTAL_REV', 'revenue', false, $sortOrder++);

        // 9. OTHERS
        $this->createCategory($propertyId, null, 'OTHERS', 'OTHER_REV', 'revenue', false, $sortOrder++);
