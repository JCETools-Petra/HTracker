                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-4 p-4 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg">
                        ...
                        <p class="text-xs text-blue-600 dark:text-blue-300 mt-1">
                            Input data realisasi (Actual) dan anggaran (Budget) untuk bulan ini. Angka akan otomatis diformat dengan pemisah ribuan.
                        </p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.financial.input-actual.store', $property->id) }}" x-data="{ activeTab: 0 }">
                        ...
                        @foreach($departments as $index => $dept)
                            <div x-show="activeTab === {{ $index }}" class="space-y-4" style="display: none;">
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach($dept['categories'] as $cat)
                                                @php
                                                    $actualVal = $existingEntries[$cat['id']]->actual_value ?? 0;
                                                    $budgetVal = $existingEntries[$cat['id']]->budget_value ?? 0;
                                                    $uniqueId = $cat['id'] . '_' . $loop->parent->index . '_' . $loop->index;
                                                @endphp
                                                <tr class="{{ $cat['is_payroll'] ? 'bg-yellow-50 dark:bg-yellow-900' : '' }}">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <input 
                                                            type="hidden" 
                                                            id="actual_hidden_{{ $uniqueId }}"
                                                            name="entries[{{ $loop->parent->index }}_{{ $loop->index }}][actual_value]" 
                                                            value="{{ $actualVal }}">
                                                        
                                                        <input
                                                            type="text"
                                                            value="{{ number_format($actualVal, 0, ',', '.') }}"
                                                            oninput="formatMoney(this, 'actual_hidden_{{ $uniqueId }}')"
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <input 
                                                            type="hidden" 
                                                            id="budget_hidden_{{ $uniqueId }}"
                                                            name="entries[{{ $loop->parent->index }}_{{ $loop->index }}][budget_value]" 
                                                            value="{{ $budgetVal }}">

                                                        <input
                                                            type="text"
                                                            value="{{ number_format($budgetVal, 0, ',', '.') }}"
                                                            oninput="formatMoney(this, 'budget_hidden_{{ $uniqueId }}')"
                                                            class="w-full text-right border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
                                                        >
                                                    </td>
                                                </tr>
